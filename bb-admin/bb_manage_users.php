...
if ($main->button(1)) //postback add new user
    ...
    if (empty($arr_error))
        ...
        $query = "INSERT INTO users_table (username, email, hash, salt, userroles, fname, minit, lname, notes, ips) " .
                 "SELECT '" . pg_escape_string($username_work) . "', '" . pg_escape_string($email_work) . "', '" . hash('sha512', pg_escape_string($passwd_work) . $salt) . "', '" . $salt . "', '{" . implode(",", array_map('pg_escape_string', $userroles_work)) . "}', " .
...
if ($main->button(3)) //postback delete_user
    ...
    if ($cnt == 0)
        ...
        {
        //since action is zero, only email needs to be blanked        
        array_push($arr_messages, "User " . $email_work . " has been locked.");
        //dispose of state    
        $arr_state = array();
        $action = $main->set('action', $arr_state, 0);
        $id = $main->set('id', $arr_state, 0);
...
if ($main->button(4)) //postback delete_user
    ...
    if ($cnt == 0)
        ...
        {
        //since action is zero, only email needs to be blanked        
        array_push($arr_messages, "User " . $email_work . " has been deleted.");
        $arr_state = array();
        $action = $main->set('action', $arr_state, 0);
        $id = $main->set('id', $arr_state, 0);
        $usersort = $main->set('usersort', $arr_state, "lname");
...
if (in_array($action, array(1,2,3,4))):
    ...
    $main->echo_input("passwd_work", "", array('type'=>'password','input_class'=>'spaced long','handler'=>$handler,'maxlength'=>$maxinput));
    echo "</div>";
    ...
    $main->echo_input("repasswd_work", "", array('type'=>'password','input_class'=>'spaced long','handler'=>$handler,'maxlength'=>$maxinput));
    echo "</div>";
