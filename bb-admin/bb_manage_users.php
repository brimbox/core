...
if ($main->button ( array (
	...
	if (! empty ( $userroles_work )) {
		if ($userrole_default != "0_bb_brimbox") {
			$userroles_work = array_diff ( $userroles_work, array (
					$userrole_default 
			) );
			array_unshift ( $userroles_work, $userrole_default );
		}
	} else {
		$userroles_work = array (
				$userrole_default 
		);
	}
...
if (in_array ( $action, array (
	...
	if (in_array ( $action, array (
			...
			2 
	) ) && $main->button ( 0 ) && ($submit == "bb_manage_users"))) {
		// edit or delete
	...
	$readonly = in_array ( $action, array (
			3,
			4 
	) ) ? true : false;
